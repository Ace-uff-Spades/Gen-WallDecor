      ...
      {loading ? (
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={chartData} barSize={chartData.length > 14 ? 8 : 16}>
            ...
            <Tooltip formatter={(v) => formatTokens(typeof v === 'number' ? v : 0)} />
            <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 12 }} />
            <Bar dataKey="gptTokens" name="GPT-4o-mini" stackId="a" fill="#6366f1" radius={[0, 0, 0, 0] as [number, number, number, number]} />
            <Bar dataKey="geminiTokens" name="Gemini" stackId="a" fill="#10b981" radius={[3, 3, 0, 0] as [number, number, number, number]} />
          </BarChart>
        </ResponsiveContainer>
      )}
    </div>
